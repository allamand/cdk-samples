...
import { DEFAULT_DOMAIN_ZONE, DEFAULT_EXTERNAL_DNS_POLICY } from '../defaults';
...
export class K8sAddsOns extends Construct {
    ...
    constructor(scope: Construct, id: string, cluster: Cluster, props: cdk.StackProps = {}) {
        ...
        new cdk.CfnOutput(this, 'AwsLoadBalancerControllerRoleOutput', { value: awsLoadBalancerController.sa.role.roleName });
        ...
        const cassandraNotetool = new EksUtilsAdmin(this, 'eksutils-admin-fargate', cluster, {
            namespace: 'cassandra',
            schedulerName: 'fargate',
            image: 'cassandra',
            command: '/bin/sh',
            //      args: ["-c", "while true; do sleep 60; done"]
            args: ["-c", "while true; do echo -n \"[$(date)] Number of Cassandra Pods Down in the cluster (Allowed by PDB: 1): \" | tee -a /var/log/containers/nodetool.log ; nodetool -h cassandra-demo.cassandra status | grep DN | wc -l | tee -a /var/log/containers/nodetool.log ; sleep 5 ; done"],
        });
